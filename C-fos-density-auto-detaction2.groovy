...
for (entry in getProject().getImageList()) {
    try {
        ...
        // ===== ✅ 修复：PAG 父 annotation 也纳入计数 =====
        // 原来只处理 leaf annotations（没有子 annotation 的区域），PAG 若是父区会被漏掉。
        // 现在：叶子区域照常算 + 名字/类别里含 "PAG" 的 annotation（即便有子区）也强制算。
        def allAnns = hierarchy.getAnnotationObjects()
        def annsToProcess = allAnns.findAll { ann ->
            if (ann.getROI() == null) return false

            def hasChildAnn = ann.getChildObjects().any { it instanceof PathAnnotationObject }
            def annNameUp = (ann.getName() ?: ann.getPathClass()?.getName() ?: "").toUpperCase()

            return !hasChildAnn || annNameUp.contains("PAG")
        }

        for (ann in annsToProcess) {
            def roi = ann.getROI()
...
print "✅ 已修复 PAG 漏算：叶子区域 + 名字/类别含 PAG 的父区域都会计数；面积仍按你的公式计算。"
